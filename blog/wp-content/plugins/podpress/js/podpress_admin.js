...
function podPressDetectDuration(VarNum, enc_or_esc) {
	...
	switch (enc_or_esc) {
		...
		//~ break;
		case 'encodeURIComponent':
			//~ podPressHttp_DetectDuration.open("GET", podPressBackendURL+'podpress_backend.php?action=getduration&filename='+escape(encodeURIComponent(params)), true);
		//~ break;
		default:
		case 'escape':
...
function podPressLoadFileInfo(VarNum, enc_or_esc) {
	...
	//	podPressLoadedMP3Info = mp3filename; // try to retrieve the ID3tag info only once (when the filename changes)
		...
		switch (enc_or_esc) {
			...
			//~ break;
			case 'encodeURIComponent':
				//~ podPressHttp_LoadFileInfo.open("GET", podPressBackendURL+'podpress_backend.php?action=showID3Contents&filename='+escape(encodeURIComponent(mp3filename))+'&keynum='+VarNum, true);
			//~ break;
			default:
			case 'escape':
