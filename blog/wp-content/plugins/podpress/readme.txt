...
podPress 8.8.8.5 is a further maintenance release which includes bug fixes. Read more about this version in the [Changelog](http://wordpress.org/extend/plugins/podpress/changelog/) and the [Upgrade](http://wordpress.org/extend/plugins/podpress/other_notes/) section. Due to the major modifications of the widgets, you will need to reconfigure them if you have used them already. It might be also necessary to check and adjust the settings i.e. of the feed with the slug name "podcast".
...
= v8.8.8.4 =
* bug fix: The file type filter works now as intended: For instance, if you have posts with a mp3 and a m4a file and you have a Feed for the posts with mp3 files and one for posts with m4a files then such post with files of both types will show up in both Feeds. In the mp3-Feed with the mp3 file as the enclosure and in the m4a-Feed with the m4a file as the enclosure. NOTICE: If you want to have such Feed combination then one of the Feeds needs to should be an ATOM Feed (You can define the type of a podPress Feed in the section with this name on the Feed/iTunes Settings page.)
...
== Upgrade Notice ==
Upgrading from 8.8.6.3 (or older version) to 8.8.8.x:
