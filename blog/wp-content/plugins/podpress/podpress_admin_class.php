	...
	class podPressAdmin_class extends podPress_class
	{
		...
		function post_form($entryType = 'post') {
			...
			while ($num < $this->settings['maxMediaFiles']) {
				...
				echo "\n";
				echo "podPressAddMediaFile(".$post->podPressMedia[$num]['showme'].", '".$post->podPressMedia[$num]['URI']."', '".$post->podPressMedia[$num]['URI_torrent']."', '".js_escape($post->podPressMedia[$num]['title'])."', '".$post->podPressMedia[$num]['type']."', '".$post->podPressMedia[$num]['size']."', '".$post->podPressMedia[$num]['duration']."', '".$post->podPressMedia[$num]['dimensionW']."', '".$post->podPressMedia[$num]['dimensionH']."', '".$post->podPressMedia[$num]['previewImage']."', ".$post->podPressMedia[$num]['rss'].", ".$post->podPressMedia[$num]['atom'].", ".$post->podPressMedia[$num]['feedonly'].", ".$post->podPressMedia[$num]['disablePlayer'].", '".$post->podPressMedia[$num]['content_level']."');\n";
				$num++;
			}
			echo '</script>'."\n";
			...
			while ($num < $this->settings['maxMediaFiles']) {
				...
				if(!empty($files) || $this->settings['enablePodangoIntegration']) {
					...
					foreach ($files as $key=>$val) {
						...
						if ( TRUE == version_compare(PHP_VERSION, '5.0', '<') ) { // html_entity_decode can not handle the charset UTF-8 in most of the PHP 4 version
							$fileOptionList .= '							<option value="'.attribute_escape(html_entity_decode(htmlentities($key))).'"'.$xSelected.'>'.html_entity_decode(htmlentities(podPress_stringLimiter($val, 45, true))).'</option>'."\n";
						} else {
							$fileOptionList .= '							<option value="'.attribute_escape(html_entity_decode(htmlentities($key), ENT_COMPAT, $blog_charset)).'"'.$xSelected.'>'.html_entity_decode(htmlentities(podPress_stringLimiter($val, 45, true)), ENT_COMPAT, $blog_charset).'</option>'."\n";
						}
					}
					if(!$fileSelected) {
						if ( TRUE == version_compare(PHP_VERSION, '5.0', '<') ) { // html_entity_decode can not handle the charset UTF-8 in most of the PHP 4 version
							echo '							<option value="'.attribute_escape(html_entity_decode(htmlentities($thisMedia['URI']))).'" selected="selected">'.html_entity_decode(htmlentities(podPress_stringLimiter($thisMedia['URI'], 45, true))).'</option>'."\n";
						} else {
							echo '							<option value="'.attribute_escape(html_entity_decode(htmlentities($thisMedia['URI']), ENT_COMPAT, $blog_charset)).'" selected="selected">'.html_entity_decode(htmlentities(podPress_stringLimiter($thisMedia['URI'], 45, true)), ENT_COMPAT, $blog_charset).'</option>'."\n";
						}
					}
					echo $fileOptionList;
					...
					echo '							<span id="podPressMedia_'.$num.'_URI_chrWarning" class="updated podPressMedia_URI_chrWarning"> '.sprintf(__('<strong>Notice:</strong> It is not recommended to use other than these characters: %1$s or whitespaces in file and folder names.', 'podpress'), 'abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789 .:/_-\ ').'</span>'."\n";
					echo '							<input type="hidden" id="podPressMedia_'.$num.'_cleanURI" value="yes" />'."\n";
					$enc_or_esc = 'encodeURIComponent';
				}
				echo '						</td>'."\n";
				echo '					</tr>'."\n";
				...
				echo '						</td>'."\n";
				echo '						<td>'."\n";
				echo '							<input type="text" id="podPressMedia_'.$num.'_duration" name="podPressMedia['.$num.'][duration]" size="10" value="'.$thisMedia['duration'].'" onchange="javascript: podPressMediaFiles['.$num.'][\'duration\'] = this.value;"/> <span class="podpress_abbr" title="'.__('hours:minutes:seconds - for example: 2:45:10 or 34:01 or 1:36 or 120:47 or 0:10', 'podpress').'">'.__('[hh:mm:ss]', 'podpress').'</span> <img src="'.PODPRESS_URL.'/images/ajax-loader.gif" id="podPressMedia_'.$num.'_duration_loadimg" class="podpress_ajax_loader_img" /><input type="button" id="podPressMedia_'.$num.'_duration_detectbutton" name="AutoDetect" value="'.__('Auto Detect', 'podpress').'" onclick="podPressDetectDuration('.$num.', \''.$enc_or_esc.'\');"/> ('.__('May take several seconds/minutes', 'podpress').')'."\n";
				echo '						</td>'."\n";
				echo '					</tr>'."\n";
				...
				echo '					<tr id="podPressMedia_'.$num.'_id3tag_details_row">'."\n";
				echo '						<td style="vertical-align: top;" nowrap="nowrap">'."\n";
				echo '							'.__('Tag (ID3) Info', 'podpress').":\n";
				echo '						</td>'."\n";
				echo '						<td>'."\n";
				echo '							<img src="'.PODPRESS_URL.'/images/ajax-loader.gif" id="podPressMedia_'.$num.'_id3tag_loadimg" class="podpress_ajax_loader_img" /><input type="button" name="podPressMedia['.$num.'][mp3_details_button]" id="podPressMedia_'.$num.'_mp3_detailsbutton_" value="'.__('Show','podpress').'" onclick="javascript: podPressShowHideMP3Details('.$num.', \''.$enc_or_esc.'\');"/>'."\n";
				echo '						</td>'."\n";
				echo '					</tr>'."\n";
				echo '					<tr>'."\n";
				echo '						<td colspan="2">'."\n";
			...
			if ( "post.php" === $pagenow OR "page.php" === $pagenow OR "post-new.php" === $pagenow OR "page-new.php" === $pagenow ) {
				...
				if(isset($_POST['iTunesSubtitleChoice']) AND $_POST['iTunesSummaryChoice'] AND $_POST['iTunesKeywordsChoice'] AND $_POST['iTunesAuthorChoice'] AND $_POST['iTunesExplicit'] AND $_POST['iTunesBlock']) {
					if($_POST['iTunesSubtitleChoice'] == 'Custom' && !empty($_POST['iTunesSubtitle'])) {
						$podPressPostSpecific['itunes:subtitle'] = htmlspecialchars(strip_tags(trim($_POST['iTunesSubtitle'])));
					} else {
						$podPressPostSpecific['itunes:subtitle'] = '##PostExcerpt##';
					}

					if($_POST['iTunesSummaryChoice'] == 'Custom' && !empty($_POST['iTunesSummary'])) {
						$podPressPostSpecific['itunes:summary'] = htmlspecialchars(strip_tags(trim($_POST['iTunesSummary'])));
					...
					if($_POST['iTunesAuthorChoice'] == 'Custom' && !empty($_POST['iTunesAuthor'])) {
						$podPressPostSpecific['itunes:author'] = htmlspecialchars(strip_tags(trim($_POST['iTunesAuthor'])));
					} else {
		...
		function edit_category_form($input) {
			...
			echo '					</select>'."\n";
			echo '					<input type="hidden" id="global_iTunesSummary" value="'.attribute_escape($this->settings['iTunes']['summary']).'" />'."\n";
			...
			echo '						<input type="text" name="iTunesAuthor" class="podpress_wide_text_field" size="40" id="iTunesAuthor" value="'.attribute_escape($data['iTunesAuthor']).'" onchange="javascript: podPress_updateCategoryCasting('.$main_wp_version.');"/>';
			echo '						<input type="hidden" id="global_iTunesAuthor" value="'.attribute_escape($this->settings['iTunes']['author']).'" />'."\n";
			echo '					</div>'."\n";
			...
			echo '						<input type="text" name="iTunesAuthorEmail" class="podpress_wide_text_field" size="40" id="iTunesAuthorEmail" value="'.attribute_escape($data['iTunesAuthorEmail']).'" onchange="javascript: podPress_updateCategoryCasting('.$main_wp_version.');"/>';
			echo '						<input type="hidden" id="global_iTunesAuthorEmail" value="'.attribute_escape(get_option('admin_email')).'" />'."\n";
			echo '					</div>'."\n";
			...
			echo '					</select>'."\n";
			echo '					<input type="hidden" id="global_blogname" value="'.attribute_escape(get_option('blogname')).'" /></td>'."\n";
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
			echo '			<tr>'."\n";
			echo '				<th>';
			...
			echo '					</select>'."\n";
			echo '					<input type="hidden" id="global_blogdescription" value="'.attribute_escape(get_option('blogdescription')).'" />'."\n";
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
			echo '			<tr>'."\n";
			echo '				<th>';
			...
			echo '						<input type="text" name="rss_copyright" class="podpress_wide_text_field" size="40" id="rss_copyright" value="'.attribute_escape($data['rss_copyright']).'" onchange="javascript: podPress_updateCategoryCasting('.$main_wp_version.');"/>';
			echo '					</div>'."\n";
			echo '					<input type="hidden" id="global_rss_copyright" value="'.attribute_escape($this->settings['rss_copyright']).'" />'."\n";
			...
			echo '				</td>'."\n";
			echo '				<td>';
			echo '					'.__('You can select one or more media file types and limit by this choice the number of posts which will appear in the this Feed. For instance: if you select MP4 Video then this Feed will only contain posts (of this category) with MP4 files. If a post has also files of other types then only the files of the selected type will be attached in this feed.', 'podpress').'</p><p>'.__('Hold the key [SHIFT] or [CTRL] and use the left mouse button to select more than one value.<br />Hold [CTRL] and use the left mouse button to deselect values.', 'podpress');		
			echo '				</td>'."\n";
			echo '			</tr>'."\n";
